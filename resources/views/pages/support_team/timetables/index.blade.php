...
@section('page_title', __('timetables.manage_timetables'))
@section('content')

    <div class="card">
        <div class="card-header header-elements-inline">
            <h6 class="card-title">{{ __('timetables.manage_timetables') }}</h6>
            {!! Qs::getPanelOptions() !!}
        </div>
        ...
        <div class="card-body">
            <ul class="nav nav-tabs nav-tabs-highlight">
                @if(Qs::userIsTeamSA())
                <li class="nav-item"><a href="#add-tt" class="nav-link active" data-toggle="tab">{{ __('timetables.create_timetable') }}</a></li>
                @endif
                <li class="nav-item dropdown">
                    <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown">{{ __('timetables.show_timetables') }}</a>
            ...
            <div class="tab-content">
                   ...
                   <div class="col-md-8">
                       <form class="ajax-store" method="post" action="{{ route('ttr.store') }}">
                           ...
                           <div class="form-group row">
                               ...
                               <div class="col-lg-9">
                                   <input name="name" value="{{ old('name') }}" required type="text" class="form-control" placeholder="{{ __('timetables.name_placeholder') }}">
                               </div>
                           </div>
                ...
                @foreach($my_classes as $mc)
                    <div class="tab-pane fade" id="ttr{{ $mc->id }}">                         <table class="table datatable-button-html5-columns">
                            <thead>
                            <tr>
                                <th>{{ __('timetables.serial_number') }}</th>
                                <th>{{ __('timetables.name') }}</th>
                                <th>{{ __('timetables.class') }}</th>
                                <th>{{ __('timetables.type') }}</th>
                                <th>{{ __('timetables.year') }}</th>
                                <th>{{ __('timetables.action') }}</th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach($tt_records->where('my_class_id', $mc->id) as $ttr)
                                <tr>
                                    ...
                                    <td>{{ ($ttr->exam_id) ? $ttr->exam->name : __('timetables.class_timetable') }}
                                    <td>{{ $ttr->year }}</td>
                                    <td class="text-center">
                                        <div class="list-icons">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    {{--View--}}
                                                    <a href="{{ route('ttr.show', $ttr->id) }}" class="dropdown-item"><i class="icon-eye"></i> {{ __('timetables.view') }}</a>

                                                    @if(Qs::userIsTeamSA())
                                                    {{--Manage--}}
                                                    <a href="{{ route('ttr.manage', $ttr->id) }}" class="dropdown-item"><i class="icon-plus-circle2"></i> {{ __('timetables.manage') }}</a>
                                                    {{--Edit--}}
                                                    <a href="{{ route('ttr.edit', $ttr->id) }}" class="dropdown-item"><i class="icon-pencil"></i> {{ __('timetables.edit') }}</a>
                                                    @endif

                                                    {{--Delete--}}
                                                    @if(Qs::userIsSuperAdmin())
                                                        <a id="{{ $ttr->id }}" onclick="confirmDelete(this.id)" href="#" class="dropdown-item"><i class="icon-trash"></i> {{ __('timetables.delete') }}</a>
