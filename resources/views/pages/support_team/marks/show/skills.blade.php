    ...
    <div class="col-md-6">
        <div class="card">
            <div class="card-header header-elements-inline bg-danger">
                <h6 class="card-title font-weight-bold">السمات الوجدانية</h6>
                {!! Qs::getPanelOptions() !!}
            </div>
                ...
                <form class="ajax-update" method="post" action="{{ route('marks.skills_update', ['AF', $exr->id]) }}">
                    ...
                    @foreach($skills->where('skill_type', 'AF') as $af)
                        <div class="form-group row">
                            ...
                            <div class="col-lg-6">
                                <select data-placeholder="اختر" name="af[]" id="af" class="form-control select">
                                    <option value=""></option>
                                    @for($i=1; $i<=5; $i++)
    ...
    <div class="col-md-6">
        <div class="card">
            <div class="card-header header-elements-inline bg-success">
                <h6 class="card-title font-weight-bold">المهارات الحركية النفسية</h6>
                {!! Qs::getPanelOptions() !!}
            </div>
                ...
                <form class="ajax-update" method="post" action="{{ route('marks.skills_update', ['PS', $exr->id]) }}">
                    ...
                    @foreach($skills->where('skill_type', 'PS') as $ps)
                        <div class="form-group row">
                            ...
                            <div class="col-lg-6">
                                <select data-placeholder="اختر" name="ps[]" id="ps" class="form-control select">
                                    <option value=""></option>
                                    @for($i=1; $i<=5; $i++)
